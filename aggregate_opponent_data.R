...
opp_games_season <- combined_opp_box %>%
...
combined_status_stats <- total_opp %>%
  group_by(status) %>% # Group by stats
  summarise( # Calculate split stats
    pts = sum(as.numeric(pts)),
    oreb = sum(as.numeric(oreb)),
    dreb = sum(as.numeric(dreb)),
    reb = sum(as.numeric(reb)),
...
combined_opp_stats <- bind_rows(opp_splits_team, combined_status_stats, combined_conf_stat, opp_season_team) %>%
  summarise( # Calculate rate stats
    status,
    conf,
    ppg = pts/games,
    oreb_pg = oreb/games,
  ...
  mutate(
    team = "Opponent" # Set the team equal to "Opponent"
  )